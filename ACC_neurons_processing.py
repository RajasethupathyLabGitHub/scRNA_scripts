...
adataN = sc.read(directory / "adata_ACC_neurons.h5ad")
...
# label cortical layer marker genes
layer_markers = pd.read_csv(directory / 'marker_layers.csv')

# Build {set_name -> unique uppercase genes}, dropping NaNs/empties
neuron_dic = {col: sorted({str(g).strip().upper() for g in layer_markers[col].dropna() if str(g).strip()})
    for col in layer_markers.columns}
...
# threshold
t = 0.2
layer_2_3  = adataN.obs['layer 2_3 markers'].astype(float) > t
layer_6 = adataN.obs['layer 6 markers'].astype(float) > t
layer_5   = adataN.obs['layer 5 markers'].astype(float) > t

adataN.obs['layers'] = np.select([layer_2_3, layer_6, layer_5],['layer 2_3', 'layer_6', 'layer_5'],default='unassigned')


## Differential gene expression with MAST - between conditions /  for each time point
...
%R pairwise_de(df1, df2, paste0(directory / "MAST_RecentRetHR_vs_RecentRetLR.csv")

## Gseapy - Enrichr analysis
gseapy.get_library_name(organism = 'Mouse')
               
gene_list = p.read_csv(directory / 'MASTsorted_RecentRetHR_vs_RecentRetLR.csv')

gseapy_res = gseapy.enrichr(gene_list=gene_list[:300], #top DEGs
                            organism='Mouse',
                            gene_sets='GO_Biological_Process_2023', 
                            cutoff = 0.5)
gseapy_res.results.to_csv(directory / 'GO_RecentRetHR_vs_RecentRetLR.csv')

### heatmap DEGs
# Load DEG table
ACC_degs = pd.read_csv(directory / 'DEGtimpoints_ACC.csv')

# Build dictionary of {group -> non-nan unique genes}
toplot_dic = {
    col: [g for g in pd.unique(ACC_degs[col].dropna()) if isinstance(g, str)]
    for col in ACC_degs.columns}

# Flatten all genes and build group labels
allgenes = ACC_degs.values.flatten(order='F')
allgenesgroup = np.repeat(ACC_degs.columns, ACC_degs.shape[0])
...
g.map(sns.kdeplot, 'avg_expression',color='blue',
